...
function RenderIssue ({issue,i}) {
    ...
    return (
            ...
            <td>
                {new Intl.DateTimeFormat('vi-VN',{year: 'numeric', month: 'short', day: '2-digit'}).format(new Date( Date.parse(issue.createdAt)))}
            </td>
            <td>
                {new Intl.DateTimeFormat('vi-VN',{year: 'numeric', month: 'short', day: '2-digit'}).format(deadline)}
            </td>
            <td>
                {
        issue.returned?('Đã trả vào '+(new Intl.DateTimeFormat('vi-VN',{year: 'numeric', month: 'short', day: '2-digit'}).format(new Date( Date.parse(returnDate)))))                    
               :('Chưa trả')
                }
            </td>
            <td>
